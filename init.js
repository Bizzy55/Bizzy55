...
/* CONFIG */
var empty_result_msg = 'There is no data available for this UK region';
...
var gender_mail_color = '#456794';
var gender_femail_color = '#a6cee3';
var UK_color = '#456794';
var NI_color = '#456794';
var currency = ''; 
...
var ajax = $.get(
	...
	function(result){
		if(count == 0){
			...
			if(typeof result.series != 'undefined' && result.series.length > 0)
				result_data['UK']['pay'] = result.series[result.series.length-1]['estpay'];
			read_ashe(3,ashe_pay_url,{soc:data.soc,filters:"region:12"},result_data);
		}else if(count == 3){
			if(typeof result.series != 'undefined' && result.series.length > 0)
				result_data['NI']['pay'] = result.series[result.series.length-1]['estpay'];
...
function create_hours_earnings(data){
	...
	html += '<tr>';
	html += '<th>&nbsp;</th>';
	html += '<th>Average Hours per Week</th>';
	
	html += '<th>Average Annual Pay (&pound;)</th>';
	html += '</tr>';
	html += '<tr>';
...
if(data[country].length && data['labels'].length > 0){
	...
	new Chart($('#job_future_chart_'+country).get(0).getContext("2d"),{
		...
		data:{
			...
			datasets: [
			{
				label: (country == 'NI')?'Northern Ireland':'UK',
				fill: true,
...
function create_industry(data){
	...
	if(data.length>0){
		$_industry.html('<table CELLSPACING=0><tr><th>&nbsp;</th><th>Industry</th><th>Jobs(%)</th></tr>'+html+'</table>');
	}else{
		$_industry.html(empty_result_msg);
	}
}
